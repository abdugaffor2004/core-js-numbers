...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const x0 = x2 - x1;
  const y0 = y2 - y1;
  const pointsDistance = Math.sqrt(x0 * x0 + y0 * y0);
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const angleOfVectors = scalarProduct / (vector1Length * vector2Length);
  return Math.acos(angleOfVectors);
}
...
function getLastDigit(value) {
  // const numberArrayOfStrings = value.toString().split('');
  // const lastDigitAsString = numberArrayOfStrings.slice(-1);
  // const lastDigitAsInteger = parseInt(lastDigitAsString, 10);
  // return lastDigitAsInteger;
  return value % 10;
}
...
function roundToPowerOfTen(num, pow) {
  const decimalNumber = num / 10 ** pow;
  const roundedDecimalNumber = Math.round(decimalNumber);
  const roundedNumber = roundedDecimalNumber * 10 ** pow;
...
function getCountOfOddNumbers(number) {
  let oddCount = 0;
  const absoluteNumber = Math.abs(number); // Абсолютное число (независимое от знака числа) - это решает проблему если входное значение отрицательное число
